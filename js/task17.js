...
function graTimeChange() {
    // 确定是否选项发生了变化          暂时不考虑此问题
    var input = document.getElementsByTagName("input");
    for (var i = 0; i < input.length; i++) {
        if(input[i].checked ){
            nowGraTime = input[i].value;                            /*设置当前页面值*/
...
function citySelectChange() {
    // 确定是否选项发生了变化 
    if (this[this.selectedIndex].value == pageState.nowSelectCity) {       /*不需要了*/
        return;
    }
    // 设置对应数据
    pageState.nowSelectCity = this[this.selectedIndex].value;              /*设置当前页面值*/
    initAqiChartData();
    // 调用图表渲染函数
    renderChart();
}
